...
if [ "$1" = "unitd" ] || [ "$1" = "unitd-debug" ]; then
    if /usr/bin/find "/var/lib/unit/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
        ...
        if /usr/bin/find "/docker-entrypoint.d/" -mindepth 1 -print -quit 2>/dev/null | /bin/grep -q .; then
            ...
            echo "$0: Installing wordpress if not already done..."
            echo "$0: ADMIN_USER: $ADMIN_USER"
            echo "$0: ADMIN_PASSWORD: $ADMIN_PASSWORD"
            echo "$0: ADMIN_EMAIL: $ADMIN_EMAIL"
            echo "$0: SITE_TITLE: $SITE_TITLE"
            echo "$0: SITE_URL: $SITE_URL"
            echo "$0: DB_URL: $DATABASE_URL"
            echo "$0: DB_PREFIX: $DB_PREFIX"
            cd /app && /usr/local/bin/wp core is-installed || /usr/local/bin/wp core install --url=$SITE_URL --title=$SITE_TITLE --admin_user=$ADMIN_USER --admin_password=$ADMIN_PASSWORD --admin_email=$ADMIN_EMAIL --skip-email
            wp plugin activate plugin-dev || true
            echo "$0: Finished installing wordpress"
